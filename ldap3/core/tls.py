...
class Tls(object):
    ...
    def __init__(self,
        ...
        elif ssl_context_ciphers:
            if log_enabled(ERROR):
                log(ERROR, 'cannot use SSLContext ciphers, SSLContext not available')
            raise LDAPSSLNotSupportedError('cannot use SSLContext ciphers, SSLContext not available')
        else:
    ...
    def wrap_socket(self, connection, do_handshake=False):
        ...
        if use_ssl_context:
            if self.version is None:  # uses the default ssl context for reasonable security
                ...
                if self.version == ssl.PROTOCOL_TLSv1_2:
